...
def run(model_input):
    def create_input_str(input_text: Dict[str, bytes]) -> InputBytesItem:
        input_item = InputStrItem()
    ...
    with grpc.insecure_channel(f"{HOST}:{port}") as grpc_channel:
        ...
        run_request = RunStrRequest(inputs=[create_input_str(model_input)], detect_drift=False, explain=False)
        single_response = grpc_client_stub.RunStr(run_request)
        unpack_and_report_outputs(single_response)

        LOGGER.info("~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~")

        LOGGER.info(f"Sending batch of input.")
        run_request_batch = RunStrRequest(inputs=[create_input_str(model_input) for _ in range(BATCH_SIZE)], explain=False)
        batch_response = grpc_client_stub.RunStr(run_request_batch)
...
if __name__ == "__main__":
    ...
    test_inputs = {"image": base64.b64encode(open(TEST_IMG_PATH, 'rb').read())}
